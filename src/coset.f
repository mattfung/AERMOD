...
C           WRITE Error Message: Repeat Non-repeatable Keyword
         ...
         ELSE
            IF (ARM .OR. ARM2) THEN
C              Process ARM_Ratios Option                 ---   CALL ARM_Ratios
               CALL ARM_Ratios
            ELSE
...
C        Check for Missing Mandatory Keywords
         ...
         IF (OLM .OR. PVMRM .OR. PVMRM2) THEN
C ---       Check for background Ozone options for OLM/PVMRM/PVMRM2
...
C                       Issue error message; OZONEVAL & O3VALUES for this sector
               ...
               END IF
            END IF
            IF ((PVMRM .OR. PVMRM2 .OR. OLM) .AND. ICSTAT(28).EQ.0) THEN
...
C ---    Check for pollutant ID = 'NO2' for PVMRM, PVMRM2, OLM, ARM, and ARM2 options
         IF ((PVMRM .OR. PVMRM2 .OR. OLM .OR. ARM .OR. ARM2) .AND. 
     &                                          POLLUT .NE. 'NO2') THEN
...
C              WRITE Error Message     ! Non-DEFAULT Option Conflict
               CALL ERRHDL(PATH,MODNAM,'E','204',KOPT)
            END IF
         ELSE IF (KOPT .EQ. 'PVMRM2') THEN
C           Non-Default/Beta option to use "total dispersion", 
C           instead of "relative dispersion" for STABLE conditions
            IF (BETA .AND. .NOT. DFAULT) THEN
               PVMRM2 = .TRUE.
            ELSE
C              WRITE Error Message     ! Non-DEFAULT/BETA Option Conflict
...
C -----  Add BETA option for low-wind-speed modifications, LOWWIND
         ELSE IF (KOPT .EQ. 'LOWWIND1') THEN
            IF (BETA) THEN
C ---          First check for LowWind1 with LowWind2 or LowWind3, which 
C              are incompatible.
               IF (L_LowWind2) THEN
C                 WRITE Error Message     ! Non-DEFAULT Option Conflict
                  CALL ERRHDL(PATH,MODNAM,'E','136','LW1 & LW2')
               ELSE IF (L_LowWind3) THEN
C                 WRITE Error Message     ! Non-DEFAULT Option Conflict
                  CALL ERRHDL(PATH,MODNAM,'E','136','LW1 & LW3')
               ELSE
...
C                 WRITE Error Message     ! Non-DEFAULT Option Conflict
               ...
               ELSE IF (L_LowWind3) THEN
C                 WRITE Error Message     ! Non-DEFAULT Option Conflict
                  CALL ERRHDL(PATH,MODNAM,'E','136','LW2 & LW3')
               ELSE
C ---             Non-regulatory option to include BETA modifications
C                 for low-wind-speed conditions, including increasing
C                 the minumum sigma-v value from the default of 0.2 m/s
...
C              WRITE Error Message     ! Non-DFAULT option
               CALL ERRHDL(PATH,MODNAM,'E','204',KOPT)
            END IF
C -----  Add BETA option for low-wind-speed modifications, LOWWIND3
         ELSE IF (KOPT .EQ. 'LOWWIND3') THEN
            IF (BETA) THEN
C ---          First check for both LowWind1 and LowWind2, which 
C              are incompatible.
               IF (L_LowWind1) THEN
...
C                 and issue warning message
                  L_LowWind3 = .TRUE.
C                 WRITE Warning Message   
                  CALL ERRHDL(PATH,MODNAM,'W','123','Non-DFAULT')
...
C ---             Assign default value of 1.0 for FRANMAX, the maximum 
C                 meander factor
                  FRANMAX = 1.0D0
...
C                 for FRANMAX on the LOW_WIND keyword
                  IF (.NOT. L_UserFRANmax) THEN
                     CALL ERRHDL(PATH,MODNAM,'W','133','1.00    ') 
                  END IF
...
C --- Check for conflicting NO2 options:
      IF (OLM .AND. (PVMRM .OR. PVMRM2)) THEN
         IF (PVMRM) THEN
C           WRITE Error Message       ! Can't specify OLM & PVMRM
            CALL ERRHDL(PATH,MODNAM,'E','141','OLM & PVMRM')
         ELSE
C           WRITE Error Message       ! Can't specify OLM & PVMRM2
            CALL ERRHDL(PATH,MODNAM,'E','141','OLM & PVMRM2')
         END IF
...
C        WRITE Error Message       ! Can't specify OLM & ARM2
      ...
      ELSE IF ((PVMRM .OR. PVMRM2) .AND. ARM) THEN
         IF (PVMRM) THEN
C           WRITE Error Message       ! Can't specify PVMRM & ARM
            CALL ERRHDL(PATH,MODNAM,'E','141','PVMRM & ARM')
         ELSE
C           WRITE Error Message       ! Can't specify PVMRM2 & ARM
            CALL ERRHDL(PATH,MODNAM,'E','141','PVMRM2 & ARM')
         END IF
      ELSE IF ((PVMRM .OR. PVMRM2) .AND. ARM2) THEN
         IF (PVMRM) THEN
C           WRITE Error Message       ! Can't specify PVMRM & ARM2
            CALL ERRHDL(PATH,MODNAM,'E','141','PVMRM & ARM2')
         ELSE
C           WRITE Error Message       ! Can't specify PVMRM2 & ARM2
            CALL ERRHDL(PATH,MODNAM,'E','141','PVMRM2 &ARM2')
         END IF
...
C        WRITE Error Message       ! Can't specify ARM & ARM2
      ...
      IF (PSDCREDIT .AND. .NOT.PVMRM .AND. .NOT.PVMRM2) THEN
C        WRITE Error Message       ! Can't specify PSDCREDIT without PVMRM/PVMRM2
...
C     set the option label in the file headers.
     ...
     &     SCREEN .OR. SCIM .OR. PVMRM .OR. PVMRM2 .OR.PSDCREDIT .OR. 
     &      OLM .OR. ARM2 .OR. BETA .OR. FASTAREA .OR. FASTALL .OR. 
...
C     Variable Declarations
      ...
      INTEGER :: I, IMOD, IMET, IAREA, IPRM, IPVM, IOLMD, IARM, IDEP, 
     &           NOPTS, MAXFields
...
C              both AREA and LINE options; issue ERROR message
         ...
         ELSE IF (KOPT .EQ. 'PRIME') THEN
            ...
            NOPTS = NOPTS + 1
            IPRM = I
         ELSE IF (KOPT .EQ. 'PVMRM' .OR. KOPT .EQ. 'PVMRM2') THEN
            PVMRMDBG = .TRUE.
            NOPTS = NOPTS + 1
            IPVM = I
         ELSE IF (KOPT .EQ. 'OLM') THEN
...
C     OLM, or DEPOS being used
      IF (PVMRMDBG .AND. .NOT. (PVMRM .OR. PVMRM2) ) THEN
C        Write Error Message:  PVMRM debug without PVMRM or PVMRM2 option
...
C ---       Assign default METEOR debug filename
     ...
     &       FIELD(IAREA+1) .NE. 'PVMRM2' .AND.
     &       FIELD(IAREA+1) .NE. 'OLM' .AND.
     &       FIELD(IAREA+1) .NE. 'ARM' .AND.
...
C ---       Assign default PVMRM debug filename
            ...
            END IF
         END IF
C ---    Assign default filename for RELDISP debug file for PVMRM/PVMRM2 options
...
C ---       Assign default OLM debug filename
     ...
     &       FIELD(IARM+1) .NE. 'AREA' .AND.
     &       FIELD(IARM+1) .NE. 'LINE' .AND.
     &       FIELD(IARM+1) .NE. 'PRIME' .AND.
     &       FIELD(IARM+1) .NE. 'PVMRM' .AND.
     &       FIELD(IARM+1) .NE. 'PVMRM2' .AND.
...
C        Assign value to ARM_1hr
         ARM_1hr = DNUM
C        Check range of value
         IF (ARM_1hr .LE. 0.0D0) THEN
            WRITE(DUMMY,'(''ARM_1hr <= 0'')')
            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
         ELSE IF (ARM_1hr .GT. 1.0D0) THEN
            WRITE(DUMMY,'(''ARM_1hr > 1'')')
            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
         ELSE IF (ARM_1hr .LT. 0.75D0) THEN
            WRITE(DUMMY,'(''ARM1hr < .75'')')
            CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
         ELSE IF (ARM_1hr .GT. 0.80D0) THEN
            WRITE(DUMMY,'(''ARM1hr > .80'')')
            CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
         END IF
...
C           Check range of value
            IF (ARM_1hr .LE. 0.0D0) THEN
               WRITE(DUMMY,'(''ARM_Ann <= 0'')')
               CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
            ELSE IF (ARM_1hr .GT. 1.0D0) THEN
               WRITE(DUMMY,'(''ARM_Ann > 1'')')
               CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
            ELSE IF (ARM_1hr .LT. 0.75D0) THEN
               WRITE(DUMMY,'(''ARM_Ann <.75'')')
               CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
            ELSE IF (ARM_1hr .GT. 0.80D0) THEN
               WRITE(DUMMY,'(''ARM_Ann >.80'')')
               CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
            END IF
         END IF
...
C        Check range of value
         ...
         ELSE IF (ARM2_Min .GT. 1.0D0) THEN
            ...
            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
         ELSE IF (ARM2_Min .LT. 0.20D0) THEN
            WRITE(DUMMY,'(''ARM2Min <0.2'')')
            CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
...
C        Error Message: Too Few Parameters
         CALL ERRHDL(PATH,MODNAM,'E','201',KEYWRD)
         GO TO 999
      ELSE IF (.NOT.L_LowWind2 .AND. .NOT.L_LowWind3 .AND. 
     &                                                IFC .GT. 4) THEN
C        Error Message: Too Many Parameters
         CALL ERRHDL(PATH,MODNAM,'E','202',KEYWRD)
         GO TO 999
