...
C --- Calculate time scale (s) for random dispersion, based on 24 hours
      IF( L_LowWind2 )THEN
C ---    Use BIGT=12 instead of 24 for LowWind2 and option
         TRAN = 12.0D0 * 3600.0D0         
      ELSEIF( L_LowWind3 )THEN
C ---    Revert to original BIGT=24 for LowWind3 option
         TRAN = BIGT * 3600.0D0
      ELSE
         TRAN = BIGT * 3600.0D0
      ENDIF
...
C --- Calculate travel time, TTRAV
      IF( L_LowWind2 )THEN
C        Use UMEAN instead of UEF with LOWWIND2 option
         TTRAV = DISTR/UMEAN
      ELSEIF( L_LowWind3 )THEN
C        Use Downwind Distance (X) instead of DISTR with LOWWIND3 option
         TTRAV = MAX( X, 1.0D0 )/UMEAN
      ELSE
C        Use original formulation based on DISTR and UEF
         TTRAV = DISTR/UEF
      ENDIF

      SIGRAN = 2.0D0*SVEF*SVEF + 
     &         UMEAN*UMEAN*(1.0D0-DEXP(-(TTRAV/TRAN)))
      FRAN   = MIN( 1.0D0, SIGRAN/TOTKIN )

      IF( L_LowWind2 .OR. L_LowWind3 )THEN
C ---    Apply upper bound on FRAN under LOWWIND2 and LOWWIND3 
C        options; with a default value of 0.95, unless modified 
C        by the user under the CO LOW_WIND keyword.
C ---    Issue informational messages for FRAN > FRANMAX
         IF( FRAN .GT. FRANMAX )THEN
...
C                 Arrays for the current receptor for the ARM, ARM2, 
...
C
C        CALLED FROM:   ARM_CALC, ARM2_CALC, OLM_CALC and PVMRM_CALC
C
C***********************************************************************
