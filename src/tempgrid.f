...
C     be less than a minimum value (0.002, defined by SPTGMN in MAIN1.INC)
      IF( STABLE )THEN
         DO NLVL = 1, NTGLVL
...
C           unless observed temperature profile is available
     ...
     &               DLOG((UREFHT-5.*SFCZ0)/SFCZ0) )**0.333333D0
            ...
            THSTAR0 = 1.4D0* (0.005D0/(ATAN(0.17D0*(2.0D0*NCLOUD8 + 
     &                                                1.0D0))) + 0.05D0)
...
C           value of XVAL is assigned in MODULE MAIN1
            VALUE  = XVAL
