...
C     Write surface file record, depending on whether deposition is included
     ...
     &      IPCODE, PRATE, RH, SFCP, NCLOUD
      ELSE
         WRITE(ISUNIT,9026) IYEAR, IMONTH, IDAY, JDAY, IHOUR,
     &      SFCHF, USTAR, WSTAR, VPTGZI, ZICONV, ZIMECH, OBULEN,
     &      SFCZ0, BOWEN, ALBEDO, UREF, WDREF, UREFHT, TA, TREFHT
      END IF
...
C     Change the wind direction from 0.0 to 360.0 if wind speed is nonzero
      ...
      IF( PFLWS(LEVEL) .GT. 0.0D0  .AND.  PFLWD(LEVEL) .EQ. 0.0D0 )THEN
         PFLWD(LEVEL) = 360.0D0

      ELSEIF( PFLWD(LEVEL) .GT. 900.0D0 )THEN
         PFLWD(LEVEL) = -999.0D0

      ELSEIF( PFLWS(LEVEL) .EQ. 0.0D0  .AND.
...
C     Ssve gridded profile arrays to 'rural' arrays (1:MXGLVL)
      GRDSVR(:) = GRIDSV(:)
      GRDSWR(:) = GRIDSW(:)
      GRDTGR(:) = GRIDTG(:)
      GRDPTR(:) = GRIDPT(:)
